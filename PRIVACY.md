...
* **Hashed IP Addresses:** Collected strictly for security purposes, specifically for rate-limiting to prevent database abuse and ensure server stability. I do not store the plain-text IP address.

> **Note:** I do not collect your real name, physical address, private chat logs, or any other sensitive personal information.

## 2. How I Protect Your Data
I use advanced cryptographic techniques to ensure your data remains secure and anonymous. I never store your Minecraft UUID, Server Address, Email Address, Group ID, or IP Address in plain text. Here is exactly how your data is protected:

* **Irreversible Hashing (Minecraft UUIDs, Email Addresses, Server Addresses, Group IDs, and IP Addresses):** The moment this data reaches my database, it is passed through a one-way cryptographic hashing algorithm (SHA-256) combined with a secret, server-side key (a "pepper"). This scrambles the data into a random string. This process is irreversible; even if my database were compromised, no one could reveal your original identifiers. For IP addresses, these hashes are also stored strictly on a temporary basis and are automatically purged when the rate-limit window expires.
...
* Protecting the database from spam, DDoS attacks, and abuse via rate-limiting.
...
## 6. Contact
If you have any questions about this Privacy Policy, your data rights, or how your data is handled, please reach out via:
